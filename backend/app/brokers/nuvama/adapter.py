...
class NuvamaMarketDataAdapter(MarketDataAdapter):
    """Thin read-only adapter over the active market provider.

    Delegates to whatever provider the factory built (Nuvama when live, mock
    otherwise), so the rest of the app never depends on the concrete broker.
    """
