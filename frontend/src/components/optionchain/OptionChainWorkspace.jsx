...
/**
 * The full option-chain desk: instrument cycler, expiry picker, market stats,
 * strike window, chain table, and the floating paper-trade ticket.
 *
 * It backs both the standalone /option-chain route and the Trade tab of the
 * trading desk, so the two can never drift apart.
 *
 * @param onInstrumentChange fires whenever the visible instrument changes, so a
 *        host page can keep its own per-instrument data in step.
 * @param onOrderPlaced fires after a ticket places a paper order, so a host page
 *        can refresh its books.
 */
export default function OptionChainWorkspace({ onInstrumentChange, onOrderPlaced }) {
  ...
  // Start on the user's preferred index the first time preferences land.
  const pickedInstrument = useRef(false)
  useEffect(() => {
    if (pickedInstrument.current) return
    const preferred = INSTRUMENTS.findIndex(i => i.name === prefs.default_instrument)
    if (preferred > -1) setIdx(preferred)
  }, [prefs.default_instrument])

  useEffect(() => { onInstrumentChange?.(instrument) }, [instrument]) // eslint-disable-line react-hooks/exhaustive-deps
