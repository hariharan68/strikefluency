...
const DisciplinePage = lazy(() => import('../discipline/DisciplinePage'))
...
const TABS = [
  ...
  { key: 'advanced', label: 'Advanced Dashboard', icon: Gauge },
...
// ── Main Dashboard Page ───────────────────────────────────────────
function DashboardTabFallback({ label }) {
  return (
    <div
      aria-busy="true"
      aria-live="polite"
...
export default function DashboardPage() {
  ...
  const tab = requestedView === 'advanced' || requestedView === 'reports'
    ? requestedView
    : 'dashboard'
  ...
  useEffect(() => {
    ...
    let active = true
    Promise.allSettled([
      getAccount().then(r => { if (active) setAccount(r.data) }),
      getScore().then(r => { if (active) setScore(r.data) }),
      getTodayViolations().then(r => { if (active) setViolations(r.data || []) }),
      getRules().then(r => { if (active) setRules(r.data || []) }),
      getSummary().then(r => { if (active) setSummary(r.data) }),
      getPnlCurve().then(r => { if (active) setPnlCurve(r.data?.data || r.data || []) }),
    ]).catch(() => {})
    return () => { active = false }
  }, [tab])
  ...
  const changeTab = nextTab => {
    ...
    if (nextTab === 'dashboard') nextParams.delete('view')
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
      ...
      <DashTabs active={tab} onChange={changeTab} />

      {tab === 'reports' && (
        <Suspense fallback={<DashboardTabFallback label="reports" />}>
          <DisciplinePage />
        </Suspense>
      )}
      {tab === 'advanced' && (
        <Suspense fallback={<DashboardTabFallback label="advanced analytics" />}>
          <AdvancedDashboard />
        </Suspense>
      )}
