...
function ScoreRing({ score = 0 }) {
  ...
  return (
    <svg width={104} height={104} viewBox="0 0 104 104">
      <circle cx={cx} cy={cy} r={r} fill="none" stroke="#F0EDF1" strokeWidth={9} />
      ...
      <text x={cx} y={cy + 15} textAnchor="middle" fill="#9CA3AF" fontSize="9" fontFamily="Inter,sans-serif" fontWeight="500">
        SCORE
      </text>
    </svg>
  )
...
function RuleRow({ rule, onUpdate }) {
  ...
  return (
    <tr className="chain-row" style={{ borderBottom: '1px solid #F8F7F9' }}>
      <td style={{ padding: '11px 16px' }}>
        <div style={{ color: '#111827', fontSize: 13 }}>{label}</div>
        <div style={{ color: '#9CA3AF', fontSize: 10, marginTop: 2 }}>{rule.rule_code}</div>
      </td>
      <td style={{ padding: '11px 8px', textAlign: 'center' }}>
        <span style={{
          fontSize: 10, padding: '3px 9px', borderRadius: 20, fontWeight: 600,
          background: isEnabled ? '#dcfce7' : '#F0EDF1',
          color: isEnabled ? '#15803d' : '#9CA3AF'
        }}>{isEnabled ? 'ACTIVE' : 'OFF'}</span>
      </td>
      <td className="num" style={{ padding: '11px 8px', textAlign: 'center', color: '#111827', fontSize: 13, fontWeight: 500 }}>
        {numVal != null ? numVal : '—'}
      </td>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <Card>
        <div style={{ display: 'flex', alignItems: 'center', padding: '18px 22px', gap: 28 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ color: '#9CA3AF', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Discipline Metrics</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
              {[
                { label: 'Compliance Score', value: `${Math.round(Number(scoreVal))}%`, color: Number(scoreVal) >= 70 ? '#16a34a' : '#d97706' },
                { label: 'Disciplined Streak', value: `${streak} trades`, color: '#111827' },
                { label: 'Active Rules', value: `${rules.filter(r => r.rule_value?.enabled !== false).length} / ${rules.length}`, color: '#111827' },
              ].map(item => (
                <div key={item.label}>
      ...
      <Card>
        ...
        {rules.length === 0 ? (
          <div style={{ padding: 32, textAlign: 'center', color: '#9CA3AF', fontSize: 12 }}>No rules found. Rules are created when you first register.</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F8F7F9' }}>
                {['Rule', 'Status', 'Value', ''].map((h, i) => (
                  <th key={i} style={{ padding: '8px 16px', textAlign: i === 3 ? 'right' : i >= 1 ? 'center' : 'left', color: '#9CA3AF', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #E5E7EB' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      <Card>
        ...
        {violations.length === 0 ? (
          <div style={{ padding: 28, textAlign: 'center' }}>
            ...
            <div style={{ color: '#6B7280', fontSize: 13 }}>No violations — excellent discipline!</div>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F8F7F9' }}>
                {['Rule', 'Status', 'Time'].map(h => (
                  <th key={h} style={{ padding: '8px 16px', textAlign: 'left', color: '#9CA3AF', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #E5E7EB' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {violations.slice(0, 20).map((v, i) => (
                <tr key={i} className="chain-row" style={{ borderBottom: '1px solid #F8F7F9' }}>
                  <td style={{ padding: '9px 16px', color: '#111827', fontSize: 12 }}>{RULE_LABELS[v.rule_code] || v.rule_code}</td>
                  ...
                  <td className="num" style={{ padding: '9px 16px', color: '#9CA3AF', fontSize: 11 }}>{formatDate(v.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
