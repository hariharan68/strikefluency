...
const PanelHead = ({ title, right }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid #F0EDF1', background: '#F8F7F9' }}>
      ...
      <td style={{ padding: '9px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
          <span style={{
            ...
            background: pos.option_type === 'CE' ? '#F3EEF3' : '#fef2f2',
            color: pos.option_type === 'CE' ? '#5A3A52' : '#dc2626'
          }}>{pos.option_type}</span>
          <span className="num" style={{ color: '#111827', fontSize: 13, fontWeight: 600 }}>
            {pos.instrument} {pos.strike_price}
          </span>
          <span style={{ color: '#9CA3AF', fontSize: 11 }}>{pos.action || 'BUY'}</span>
        </div>
        <div style={{ color: '#9CA3AF', fontSize: 11, marginTop: 2 }}>{pos.quantity} lots · {pos.expiry_date || 'Weekly'}</div>
      </td>
      <td className="num" style={{ padding: '9px 10px', textAlign: 'right', color: '#6B7280', fontSize: 12 }}>{pos.entry_price?.toFixed(2)}</td>
      <td className="num" style={{ padding: '9px 10px', textAlign: 'right', color: '#6B7280', fontSize: 12 }}>{pos.current_price?.toFixed(2) ?? '—'}</td>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {INSTRUMENTS.map(ins => (
            <button key={ins} onClick={() => setInstrument(ins)}
              ...
              style={{
                ...
                background: instrument === ins ? '#714B67' : 'transparent',
                color: instrument === ins ? '#fff' : '#6B7280',
              }}>
              {ins}
            </button>
          ))}
        ...
        {open.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: '#6B7280', fontSize: 12 }}>Open P&L:</span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 14, alignItems: 'start' }}>
        <Card>
          <PanelHead
            ...
            right={<span style={{ color: '#9CA3AF', fontSize: 11 }}>Click LTP to prefill order</span>}
          />
          <OptionChainTable
        ...
        <Card>
          <PanelHead
            title="Place Order"
            right={prefill && (
              <button onClick={() => setPrefill(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9CA3AF' }}>
                <X size={14} />
              </button>
            )}
          />
          {prefill && (
            <div style={{ padding: '6px 16px', borderBottom: '1px solid #F0EDF1', background: '#F3EEF3' }}>
              <span style={{ color: '#714B67', fontSize: 11 }}>
